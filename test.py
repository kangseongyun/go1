...
if st.session_state.selected_option == "대표 솔루션":
    with st.container():
        image_html1 = image_input('효과설명3.png', '40%', '60%', '-15px', '15px', 'left')
        if image_html1:
            st.markdown(image_html1, unsafe_allow_html=True)
...
if st.session_state.selected_option == "사후 솔루션 Detail":
    with st.container():
        image_html1 = image_input('효과설명3.png', '40%', '60%', '-5px', '15px', 'left')
        if image_html1:
            st.markdown(image_html1, unsafe_allow_html=True)
...
if st.session_state.selected_option == "미래 솔루션 Detail":
    with st.container():
        ...
        if 'predict_data1' in st.session_state:
            ...
            st.markdown(
                f'<h2 style="font-size:24px; color:{title_color}; margin-top: 10px; margin-bottom: -10px; font-weight: bold;">1. 연간 예측 및 최적화 결과</h2>',
                unsafe_allow_html=True)
            st.markdown(
                f"<hr style='border: 0.5px solid {hr_color}; margin-top: -10px; margin-bottom: -10px;'>",
                unsafe_allow_html=True)
            ...
            csv = df_energy_predict.to_csv(index=False).encode('utf-8')
